...
const QUARTERS: string[][] = [
  ...
  // Q6: Industrial Refrigeration, DC Ops, Building Cx
  [
    'industrial-ref-jr-fresh.json',
    'industrial-ref-jr-derived',
    'dc-ops-jr-fresh.json',
  ...
  // Q8: Water & Wastewater Treatment Operator
  [
    'water-wastewater-jr-derived',
  ],
];
    ...
    <section className="section-pad">
      <div className="container-site max-w-5xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <div className="card-dark p-4 border-yellow-900/50">
            <h3 className="text-sm font-semibold text-white mb-2">Jr. Field PM Bank</h3>
            {statsLoading ? (
              <p className="text-xs text-gray-500">Loading…</p>
            ) : stats?.jr_field_pm ? (
              <>
                <p className="text-2xl font-bold text-yellow-400">{stats.jr_field_pm.active.toLocaleString()}</p>
                <p className="text-xs text-gray-500">active of {stats.jr_field_pm.total.toLocaleString()} total · target 1,000</p>
                <div className="mt-2 h-1.5 bg-gray-800 rounded-full">
                  <div className="h-full bg-yellow-500 rounded-full" style={{ width: `${Math.min(100, (stats.jr_field_pm.active / 1000) * 100)}%` }} />
                </div>
              </>
            ) : <p className="text-xs text-red-400">Failed to load</p>}
          </div>
          <div className="card-dark p-4 border-cyan-900/50">
            <h3 className="text-sm font-semibold text-white mb-2">Jr. Pump Tech Bank</h3>
            {statsLoading ? (
              <p className="text-xs text-gray-500">Loading…</p>
            ) : stats?.jr_pump_tech ? (
              <>
                <p className="text-2xl font-bold text-cyan-400">{stats.jr_pump_tech.active.toLocaleString()}</p>
                <p className="text-xs text-gray-500">active of {stats.jr_pump_tech.total.toLocaleString()} total · target 1,000</p>
                <div className="mt-2 h-1.5 bg-gray-800 rounded-full">
                  <div className="h-full bg-cyan-500 rounded-full" style={{ width: `${Math.min(100, (stats.jr_pump_tech.active / 1000) * 100)}%` }} />
                </div>
              </>
            ) : <p className="text-xs text-red-400">Failed to load</p>}
          </div>
          <div className="card-dark p-4 border-indigo-900/50">
            <h3 className="text-sm font-semibold text-white mb-2">Jr. Telecom OSP Bank</h3>
            {statsLoading ? (
              <p className="text-xs text-gray-500">Loading…</p>
            ) : stats?.jr_telecom_tech ? (
              <>
                <p className="text-2xl font-bold text-indigo-400">{stats.jr_telecom_tech.active.toLocaleString()}</p>
                <p className="text-xs text-gray-500">active of {stats.jr_telecom_tech.total.toLocaleString()} total · target 1,000</p>
                <div className="mt-2 h-1.5 bg-gray-800 rounded-full">
                  <div className="h-full bg-indigo-500 rounded-full" style={{ width: `${Math.min(100, (stats.jr_telecom_tech.active / 1000) * 100)}%` }} />
                </div>
              </>
            ) : <p className="text-xs text-red-400">Failed to load</p>}
          </div>
          <div className="card-dark p-4 border-emerald-900/50">
            <h3 className="text-sm font-semibold text-white mb-2">Jr. Industrial Ref Bank</h3>
            {statsLoading ? (
              <p className="text-xs text-gray-500">Loading…</p>
            ) : stats?.jr_industrial_ref ? (
              <>
                <p className="text-2xl font-bold text-emerald-400">{stats.jr_industrial_ref.active.toLocaleString()}</p>
                <p className="text-xs text-gray-500">active of {stats.jr_industrial_ref.total.toLocaleString()} total · target 1,000</p>
                <div className="mt-2 h-1.5 bg-gray-800 rounded-full">
                  <div className="h-full bg-emerald-500 rounded-full" style={{ width: `${Math.min(100, (stats.jr_industrial_ref.active / 1000) * 100)}%` }} />
                </div>
              </>
            ) : <p className="text-xs text-red-400">Failed to load</p>}
          </div>
          <div className="card-dark p-4 border-violet-900/50">
            <h3 className="text-sm font-semibold text-white mb-2">Jr. DC Ops Bank</h3>
            {statsLoading ? (
              <p className="text-xs text-gray-500">Loading…</p>
            ) : stats?.jr_dc_ops ? (
              <>
                <p className="text-2xl font-bold text-violet-400">{stats.jr_dc_ops.active.toLocaleString()}</p>
                <p className="text-xs text-gray-500">active of {stats.jr_dc_ops.total.toLocaleString()} total · target 1,000</p>
                <div className="mt-2 h-1.5 bg-gray-800 rounded-full">
                  <div className="h-full bg-violet-500 rounded-full" style={{ width: `${Math.min(100, (stats.jr_dc_ops.active / 1000) * 100)}%` }} />
                </div>
              </>
            ) : <p className="text-xs text-red-400">Failed to load</p>}
          </div>
          <div className="card-dark p-4 border-teal-900/50">
            <h3 className="text-sm font-semibold text-white mb-2">Jr. Building Cx Bank</h3>
            {statsLoading ? (
              <p className="text-xs text-gray-500">Loading…</p>
            ) : stats?.jr_building_cx ? (
              <>
                <p className="text-2xl font-bold text-teal-400">{stats.jr_building_cx.active.toLocaleString()}</p>
          ...
          <div className="card-dark p-4 border-sky-900/50">
            <h3 className="text-sm font-semibold text-white mb-2">Jr. Water/Wastewater Bank</h3>
            {statsLoading ? (
              <p className="text-xs text-gray-500">Loading…</p>
            ) : stats?.jr_water_wastewater ? (
              <>
                <p className="text-2xl font-bold text-sky-400">{stats.jr_water_wastewater.active.toLocaleString()}</p>
                <p className="text-xs text-gray-500">active of {stats.jr_water_wastewater.total.toLocaleString()} total · target 1,000</p>
                <div className="mt-2 h-1.5 bg-gray-800 rounded-full">
                  <div className="h-full bg-sky-500 rounded-full" style={{ width: `${Math.min(100, (stats.jr_water_wastewater.active / 1000) * 100)}%` }} />
                </div>
              </>
            ) : <p className="text-xs text-red-400">Failed to load</p>}
          </div>
        ...
        <div className="card-dark p-6 mb-6 border-indigo-900/50">
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            {QUARTERS.map((quarter, i) => {
              const labels = ['Q1 — Base FSE + Kitchen + HVAC + Gen + DC', 'Q2 — Solar → Pool', 'Q3 — HVAC Tech → BMET', 'Q4 — BAS → Pump Tech', 'Q5 — Telecom OSP', 'Q6 — Industrial Ref + DC Ops + Building Cx', 'Q7 — Switchgear Tech', 'Q8 — Water/Wastewater'];
              const colors = ['bg-indigo-700 hover:bg-indigo-600', 'bg-teal-700 hover:bg-teal-600', 'bg-amber-700 hover:bg-amber-600', 'bg-rose-700 hover:bg-rose-600', 'bg-cyan-700 hover:bg-cyan-600', 'bg-emerald-700 hover:bg-emerald-600', 'bg-violet-700 hover:bg-violet-600', 'bg-sky-700 hover:bg-sky-600'];
